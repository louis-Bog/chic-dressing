	...
	<div id="featured-slider" class="<?php echo esc_attr(ashe_options('general_slider_width')) === 'boxed' ? 'boxed-wrapper' : ''; ?>" data-slick="<?php echo esc_attr($slider_data); ?>">
		...
		foreach ($slider_repeater as $repeater_item) : ?>
			...
			<div class="slider-item">
				...
				<div class="slider-item-bg" style="background-image:url( <?php echo wp_get_attachment_image_src($repeater_item->image_url, 'thumbnail')[0]; ?>);"></div>
						...
						<div class="cv-inner">
							<div class="slider-info">
								...
								if ($repeater_item->btn_text === '' && $repeater_item->link !== '') {
									echo '<a aria-label="Chic Dressing" class="slider-image-link" href="' . esc_url($repeater_item->link) . '" target="' . $target . '"></a>';
								}
								...
								<?php if ($repeater_item->title !== '') : ?>
									<?php if ($repeater_item->link !== '') : ?>
										<h2 class="slider-title">
											<a aria-label="Chic Dressing" href="<?php echo esc_url($repeater_item->link); ?>"><?php echo $repeater_item->title; ?></a>
										</h2>
									<?php else : ?>
								...
								<?php if ($repeater_item->btn_text !== '') : ?>
									<div class="slider-read-more">
										<a aria-label="Chic Dressing" href="<?php echo esc_url($repeater_item->link); ?>" target="<?php echo $target; ?>"><?php echo $repeater_item->btn_text; ?></a>
									</div>
								<?php endif; ?>
